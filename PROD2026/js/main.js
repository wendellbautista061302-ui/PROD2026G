/* ===== NAVIGATION ===== */
    ...
    function saveLocally() {
        ...
        /* Push to Supabase so other PCs get updates instantly */
        if (typeof sbPushLogs === 'function') sbPushLogs();
    }
    ...
    /* ══════ JO HISTORY PANEL ══════
       Source of truth: Supabase jo_history table
    ...
    function removeJOHistoryByName(joNum, silent) {
        ...
        if (joHistory.length !== before) {
            saveHistory(); // removes from Supabase → all devices update instantly
            renderJOHistory();
            if (!silent) {
    ...
    function removeJOHistory(idx) {
        ...
        saveHistory(); // sync removal to Supabase → all devices
        renderJOHistory();
    }
...
function josSave() {
  ...
  fbPushSchedule(); // live sync to Supabase → all other PCs update instantly
}
...
function openJOScheduler() {
  josLoad(); // load from localStorage first so UI is instant
  var now = new Date();
  ...
  // Fetch latest from Supabase and re-render (non-blocking)
  if (typeof sbFetchSchedule === 'function') {
    sbFetchSchedule().then(function() { josRender(); });
  }
  if (typeof sbFetchHistory === 'function') sbFetchHistory();
  startFbListener();
}
...
function josSetStatus(id,ds,status){
  ...
  if(status==='done'){
    /* Remove from JO History on ALL devices via Supabase jo_history table */
...
function refreshFbMasterStatus(){
  // No-op: Supabase is always pre-configured
}
function saveFbMasterConfig(){ josToast('Supabase is pre-configured — no setup needed!','ok'); }
function testFbMasterConnection(){
  sbClient().from('jo_queue').select('count').then(function(r){
    josToast(r.error?'✗ Error: '+r.error.message:'✅ Supabase connection OK!', r.error?'err':'ok');
  });
}
function clearFbMasterConfig(){ josToast('Supabase config is built-in and cannot be cleared.',''); }
...
<!-- ════════════════════════════════════════════════════
     J.O. COST TRACKER — embedded in PROD2026 MASTER
     Reads: logs_v7 · stats_archive_v7 · attendance_archive_v7
     Pay field: laborCost (PROD2026 confirmed)
════════════════════════════════════════════════════ -->
<div id="joCostOverlay" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:10050;overflow-y:auto;padding:16px;box-sizing:border-box;backdrop-filter:blur(6px);">
<div style="background:#f5f6fa;width:100%;max-width:1500px;border-radius:20px;box-shadow:0 25px 60px rgba(0,0,0,.5);margin:auto;overflow:hidden;">

  <div style="background:#2c3e50;color:#fff;padding:16px 22px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;">
    <div>
      <div style="font-size:17px;font-weight:900;letter-spacing:.5px;">💰 J.O. COST TRACKER</div>
      <div style="font-size:11px;opacity:.7;margin-top:2px;">Live sync from PROD2026 scanner · <span style="color:#4ade80;">● laborCost field active</span></div>
    </div>
    <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
      <button onclick="jocRefresh()" style="background:#0f766e;color:#fff;border:none;border-radius:8px;padding:8px 14px;font-size:12px;font-weight:700;cursor:pointer;">⟳ Refresh</button>
      <button onclick="jocExportCSV()" style="background:#1d4ed8;color:#fff;border:none;border-radius:8px;padding:8px 14px;font-size:12px;font-weight:700;cursor:pointer;">⬇ CSV</button>
      <button onclick="jocOpenSheetsSettings()" style="background:#16a34a;color:#fff;border:none;border-radius:8px;padding:8px 14px;font-size:12px;font-weight:700;cursor:pointer;">📊 Sheets</button>
      <button id="joc-restore-btn" onclick="jocRestoreAll()" style="display:none;background:#dc2626;color:#fff;border:none;border-radius:8px;padding:8px 14px;font-size:12px;font-weight:700;cursor:pointer;">↺ Restore</button>
      <button onclick="closeJOCostTracker()" style="background:#dc2626;color:#fff;border:none;border-radius:8px;padding:8px 16px;font-size:14px;font-weight:900;cursor:pointer;">✕ CLOSE</button>
    </div>
  </div>

  <div style="background:#fff;padding:12px 18px;display:flex;gap:10px;flex-wrap:wrap;align-items:flex-end;border-bottom:1px solid #e8e8e8;">
    <div style="display:flex;flex-direction:column;gap:3px;"><label style="font-size:10px;font-weight:700;color:#6b7280;text-transform:uppercase;">J.O.</label><input id="joc-f-jo" type="text" placeholder="e.g. JO26042061" oninput="jocRefresh()" style="font-size:12px;padding:5px 9px;border:1px solid #d1d5db;border-radius:6px;height:30px;width:140px;"/></div>
    <div style="display:flex;flex-direction:column;gap:3px;"><label style="font-size:10px;font-weight:700;color:#6b7280;text-transform:uppercase;">Worker</label><select id="joc-f-worker" onchange="jocRefresh()" style="font-size:12px;padding:5px 9px;border:1px solid #d1d5db;border-radius:6px;height:30px;width:150px;"><option value="">All Workers</option></select></div>
    <div style="display:flex;flex-direction:column;gap:3px;"><label style="font-size:10px;font-weight:700;color:#6b7280;text-transform:uppercase;">From</label><input id="joc-f-from" type="date" onchange="jocRefresh()" style="font-size:12px;padding:5px 9px;border:1px solid #d1d5db;border-radius:6px;height:30px;"/></div>
    <div style="display:flex;flex-direction:column;gap:3px;"><label style="font-size:10px;font-weight:700;color:#6b7280;text-transform:uppercase;">To</label><input id="joc-f-to" type="date" onchange="jocRefresh()" style="font-size:12px;padding:5px 9px;border:1px solid #d1d5db;border-radius:6px;height:30px;"/></div>
    <div style="display:flex;flex-direction:column;gap:3px;"><label style="font-size:10px;font-weight:700;color:#6b7280;text-transform:uppercase;">Status</label><select id="joc-f-status" onchange="jocRefresh()" style="font-size:12px;padding:5px 9px;border:1px solid #d1d5db;border-radius:6px;height:30px;width:110px;"><option value="">All</option><option value="PASS">✅ PASS</option><option value="FAIL">❌ FAIL</option></select></div>
    <button onclick="jocClearFilters()" style="height:30px;padding:0 12px;border:1px solid #d1d5db;background:#f9fafb;border-radius:6px;font-size:12px;font-weight:600;cursor:pointer;color:#374151;">Clear</button>
  </div>

  <div id="joc-cards" style="display:flex;gap:10px;flex-wrap:wrap;padding:14px 18px;background:#fff;border-bottom:1px solid #e8e8e8;"></div>

  <div style="overflow-x:auto;padding:14px 18px;">
    <table style="border-collapse:collapse;width:100%;min-width:1100px;font-size:11px;background:#fff;border-radius:10px;overflow:hidden;border:1px solid #e8e8e8;">
      <thead>
        <tr>
          <th rowspan="2" style="background:#2c3e50;color:#fff;padding:8px 10px;text-align:left;font-size:10px;font-weight:700;text-transform:uppercase;border:1px solid #1a252f;min-width:120px;">J.O. Number</th>
          <th colspan="4" style="background:#1a4a8a;color:#fff;padding:8px;font-size:10px;font-weight:700;text-transform:uppercase;border:1px solid #1a252f;text-align:center;">CUTTING (CU)</th>
          <th rowspan="2" style="background:#0d3060;color:#fff;padding:8px;font-size:10px;font-weight:700;text-transform:uppercase;border:1px solid #1a252f;min-width:80px;text-align:center;">TOTAL CU</th>
          <th rowspan="2" style="background:#14532d;color:#fff;padding:8px;font-size:10px;font-weight:700;text-transform:uppercase;border:1px solid #1a252f;min-width:120px;text-align:center;">STAGE 1<br><span style="font-weight:400;opacity:.75;font-size:9px;">Advance</span></th>
          <th rowspan="2" style="background:#7c2d12;color:#fff;padding:8px;font-size:10px;font-weight:700;text-transform:uppercase;border:1px solid #1a252f;min-width:120px;text-align:center;">STAGE 2<br><span style="font-weight:400;opacity:.75;font-size:9px;">Assembly / Paint</span></th>
          <th rowspan="2" style="background:#581c87;color:#fff;padding:8px;font-size:10px;font-weight:700;text-transform:uppercase;border:1px solid #1a252f;min-width:120px;text-align:center;">STAGE 3<br><span style="font-weight:400;opacity:.75;font-size:9px;">Sewing / QC</span></th>
          <th rowspan="2" style="background:#7f1d1d;color:#fff;padding:8px;font-size:10px;font-weight:700;text-transform:uppercase;border:1px solid #1a252f;min-width:120px;text-align:center;">STAGE 4<br><span style="font-weight:400;opacity:.75;font-size:9px;">Packing</span></th>
          <th rowspan="2" style="background:#1a1a2e;color:#fff;padding:8px;font-size:10px;font-weight:700;text-transform:uppercase;border:1px solid #1a252f;min-width:90px;text-align:center;">J.O. TOTAL</th>
          <th rowspan="2" style="background:#16a34a;color:#fff;padding:8px;font-size:10px;font-weight:700;text-transform:uppercase;border:1px solid #1a252f;min-width:48px;text-align:center;">📊</th>
          <th rowspan="2" style="background:#dc2626;color:#fff;padding:8px;font-size:10px;font-weight:700;text-transform:uppercase;border:1px solid #1a252f;min-width:42px;text-align:center;">DEL</th>
        </tr>
        <tr>
          <th style="background:#eff6ff;color:#1d4ed8;padding:5px 6px;font-size:9px;font-weight:700;border:1px solid #ddd;text-align:center;min-width:110px;">CUS1</th>
          <th style="background:#eff6ff;color:#1d4ed8;padding:5px 6px;font-size:9px;font-weight:700;border:1px solid #ddd;text-align:center;min-width:110px;">CUS2</th>
          <th style="background:#eff6ff;color:#1d4ed8;padding:5px 6px;font-size:9px;font-weight:700;border:1px solid #ddd;text-align:center;min-width:110px;">CUS3</th>
          <th style="background:#eff6ff;color:#1d4ed8;padding:5px 6px;font-size:9px;font-weight:700;border:1px solid #ddd;text-align:center;min-width:110px;">CUS4</th>
        </tr>
      </thead>
      <tbody id="joc-tbody"><tr><td colspan="13" style="text-align:center;padding:30px;color:#6b7280;font-style:italic;">Opening… click Refresh or finish a job to load data.</td></tr></tbody>
    </table>
  </div>
</div>
</div>
...
<!-- Detail Modal -->
<div id="joc-detail-overlay" onclick="jocCloseDetail()" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:10060;"></div>
<div id="joc-detail-modal" style="display:none;position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);width:min(840px,94vw);max-height:88vh;overflow-y:auto;background:#fff;border-radius:16px;box-shadow:0 20px 60px rgba(0,0,0,.35);z-index:10061;"><div id="joc-detail-inner"></div></div>

<!-- Sheets Modal -->
<div id="joc-sheets-overlay" onclick="jocCloseSheetsSettings()" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:10070;"></div>
<div id="joc-sheets-modal" style="display:none;position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);width:min(520px,92vw);background:#fff;border-radius:16px;box-shadow:0 20px 60px rgba(0,0,0,.35);z-index:10071;padding:26px;">
  <h3 style="font-size:15px;font-weight:900;margin-bottom:8px;">📊 Google Sheets Settings</h3>
  <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:16px;">
    <div style="display:flex;flex-direction:column;gap:4px;"><label style="font-size:11px;font-weight:700;color:#374151;text-transform:uppercase;">Apps Script Web App URL</label><input id="joc-sheets-url" type="url" placeholder="https://script.google.com/macros/s/YOUR_ID/exec" style="font-size:12px;padding:8px 10px;border:1px solid #d1d5db;border-radius:6px;"/></div>
    <div style="display:flex;flex-direction:column;gap:4px;"><label style="font-size:11px;font-weight:700;color:#374151;text-transform:uppercase;">Sheet Tab Name</label><input id="joc-sheets-name" type="text" placeholder="JO Cost Tracker" style="font-size:12px;padding:8px 10px;border:1px solid #d1d5db;border-radius:6px;"/></div>
  </div>
  <div style="display:flex;gap:8px;flex-wrap:wrap;">
    <button onclick="jocSaveSheetsSettings()" style="background:#16a34a;color:#fff;border:none;border-radius:8px;padding:9px 16px;font-size:13px;font-weight:700;cursor:pointer;">💾 Save</button>
    <button onclick="jocCopyScript()" style="background:#7c3aed;color:#fff;border:none;border-radius:8px;padding:9px 16px;font-size:13px;font-weight:700;cursor:pointer;">📋 Copy Script</button>
    <button onclick="jocCloseSheetsSettings()" style="border:1px solid #d1d5db;background:#fff;border-radius:8px;padding:9px 16px;font-size:13px;font-weight:600;cursor:pointer;">Cancel</button>
  </div>
</div>
...
/* ════════════════════════════════════════════════════════════════
   PROD2026 — LIVE SYNC PRESENCE SYSTEM
   Shows which PCs are connected RIGHT NOW using Supabase Realtime
   Broadcast. Each PC announces itself and heartbeats every 10s.
════════════════════════════════════════════════════════════════ */

/* ── Generate a unique ID for THIS PC (persisted in localStorage) ── */
var _myPcId   = localStorage.getItem('prod2026_pc_id');
if (!_myPcId) {
  _myPcId = 'PC-' + Math.random().toString(36).slice(2,6).toUpperCase();
  localStorage.setItem('prod2026_pc_id', _myPcId);
}
var _myPcName = localStorage.getItem('prod2026_pc_name') || _myPcId;
var _presenceCh  = null;   // Realtime presence channel
var _onlinePeers = {};     // { pcId: { name, lastSeen, ping } }
var _sbLastSyncTime = null;

/* ── Start presence channel ── */
function sbStartPresence() {
  if (_presenceCh) return;
  var sb = sbClient();
  if (!sb) return;

  _presenceCh = sb.channel('prod2026_presence', {
    config: { presence: { key: _myPcId } }
  });
  ...
  _presenceCh
    ...
    .on('presence', { event: 'sync' }, function() {
      ...
      Object.keys(state).forEach(function(key) {
        ...
        var p = arr[0];
        if (p.pcId === _myPcId) return; // skip self
        _onlinePeers[p.pcId] = { name: p.pcName || p.pcId, lastSeen: Date.now(), ping: p.ping || null };
      });
      _renderPeers();
      _updateNavBadge();
    })
    /* Broadcast messages (test ping, sync notifications) */
    .on('broadcast', { event: 'ping' }, function(msg) {
      var from = msg.payload && msg.payload.from ? msg.payload.from : 'Unknown';
      var fromName = msg.payload && msg.payload.name ? msg.payload.name : from;
      /* Reply so they know we're here */
      _presenceCh.send({ type: 'broadcast', event: 'pong',
        payload: { from: _myPcId, name: _myPcName, to: from } });
      /* Show toast */
      var toastFn = typeof josToast === 'function' ? josToast : function(m) { console.log(m); };
      toastFn('📡 Ping from ' + fromName + '!', 'ok');
    })
    .on('broadcast', { event: 'pong' }, function(msg) {
      var fromName = msg.payload && msg.payload.name ? msg.payload.name : 'Another PC';
      var toastFn = typeof josToast === 'function' ? josToast : function(m) { console.log(m); };
      toastFn('✅ ' + fromName + ' replied — LIVE sync confirmed!', 'ok');
    })
    .on('broadcast', { event: 'data_updated' }, function(msg) {
      _sbLastSyncTime = new Date();
      _renderLastSync();
      var toastFn = typeof josToast === 'function' ? josToast : function(m){};
      if (msg.payload && msg.payload.type) {
        toastFn('🔄 ' + (msg.payload.from_name||'Another PC') + ' updated ' + msg.payload.type, 'ok');
      }
    })
    .subscribe(function(status) {
      if (status === 'SUBSCRIBED') {
        /* Announce ourselves */
        _presenceCh.track({ pcId: _myPcId, pcName: _myPcName, joinedAt: new Date().toISOString() });
        console.log('[SB Presence] Announced as', _myPcName);
      }
    });
}

/* ── Broadcast that we just changed data ── */
function sbBroadcastUpdate(type) {
  if (!_presenceCh) return;
  _sbLastSyncTime = new Date();
  _renderLastSync();
  _presenceCh.send({ type: 'broadcast', event: 'data_updated',
    payload: { from: _myPcId, from_name: _myPcName, type: type, ts: Date.now() } });
}

/* ── Send a test ping to all connected PCs ── */
function sbSendTestPing() {
  if (!_presenceCh) { alert('Not connected to Supabase yet.'); return; }
  _presenceCh.send({ type: 'broadcast', event: 'ping',
    payload: { from: _myPcId, name: _myPcName } });
  var toastFn = typeof josToast === 'function' ? josToast : function(m){};
  var count = Object.keys(_onlinePeers).length;
  if (count === 0) toastFn('⚠️ No other PCs online right now', '');
  else toastFn('📡 Ping sent to ' + count + ' PC(s)…', 'ok');
}

/* ── Render the peers list in the modal ── */
function _renderPeers() {
  var list  = document.getElementById('sb-peers-list');
  var empty = document.getElementById('sb-peers-empty');
  if (!list) return;
  var peers = Object.keys(_onlinePeers);
  if (!peers.length) {
    list.innerHTML = '<div style="text-align:center;padding:18px;color:#475569;font-size:13px;font-weight:600;">📵 No other PCs connected right now<br><span style="font-size:11px;color:#334155;font-weight:400;margin-top:4px;display:block;">Open the file on another PC to see it appear here</span></div>';
    return;
  }
  list.innerHTML = peers.map(function(id) {
    var p = _onlinePeers[id];
    var ago = Math.round((Date.now() - p.lastSeen) / 1000);
    var agoStr = ago < 5 ? 'just now' : ago + 's ago';
    return '<div class="sb-peer-card">'
      + '<div style="width:12px;height:12px;border-radius:50%;background:#22c55e;flex-shrink:0;box-shadow:0 0 8px #22c55e;animation:sbPulse 2s infinite;"></div>'
      + '<div style="flex:1;">'
      + '<div style="font-size:14px;font-weight:800;color:#f1f5f9;">' + (p.name || id) + '</div>'
      + '<div style="font-size:11px;color:#475569;margin-top:2px;">🟢 Online · seen ' + agoStr + '</div>'
      + '</div>'
      + '<div style="font-size:11px;color:#0ea5e9;font-weight:700;background:rgba(14,165,233,0.1);padding:4px 10px;border-radius:8px;">LIVE ✓</div>'
      + '</div>';
  }).join('');
}

/* ── Update the nav button badge with peer count ── */
function _updateNavBadge() {
  var btn = document.getElementById('sb-status-btn');
  if (!btn) return;
  var count = Object.keys(_onlinePeers).length;
  var led   = document.getElementById('sb-status-led');
  if (count > 0) {
    btn.style.background = '#16a34a';
    if (led) { led.style.background = '#22c55e'; led.style.boxShadow = '0 0 10px #22c55e'; }
    /* Show peer count badge */
    var badge = btn.querySelector('.sb-peer-badge') || (function(){
      var b = document.createElement('span');
      b.className = 'sb-peer-badge';
      b.style.cssText = 'background:#fff;color:#16a34a;font-size:10px;font-weight:900;padding:2px 7px;border-radius:10px;margin-left:4px;';
      btn.appendChild(b); return b;
    })();
    badge.textContent = count + ' PC' + (count>1?'s':'') + ' LIVE';
  } else {
    btn.style.background = '#0ea5e9';
    if (led) { led.style.background = '#0ea5e9'; led.style.boxShadow = '0 0 8px #0ea5e9'; }
    var badge2 = btn.querySelector('.sb-peer-badge');
    if (badge2) badge2.remove();
  }
}

function _renderLastSync() {
  var el = document.getElementById('sb-last-sync');
  if (!el || !_sbLastSyncTime) return;
  el.textContent = _sbLastSyncTime.toLocaleTimeString();
  el.style.color = '#22c55e';
}

/* ── Open / close modal ── */
function openSbStatusModal() {
  document.getElementById('sb-status-overlay').style.display = 'flex';
  runSbDiagnostic();
}
function closeSbStatusModal() {
  document.getElementById('sb-status-overlay').style.display = 'none';
}

/* ── Set PC name ── */
function sbSetMyName(name) {
  if (!name) return;
  _myPcName = name;
  localStorage.setItem('prod2026_pc_name', name);
  if (_presenceCh) _presenceCh.track({ pcId: _myPcId, pcName: _myPcName, joinedAt: new Date().toISOString() });
}

/* ── Full diagnostic + presence render ── */
async function runSbDiagnostic() {
  /* Update MY PC bar */
  var myBar  = document.getElementById('sb-mypc-bar');
  var myLbl  = document.getElementById('sb-mypc-label');
  var mySub  = document.getElementById('sb-mypc-sub');
  var myDot  = document.getElementById('sb-mypc-dot');
  var pingEl = document.getElementById('sb-ping-val');
  var rtEl   = document.getElementById('sb-rt-val');
  var schedEl= document.getElementById('sb-sched-val');
  var histEl = document.getElementById('sb-hist-val');

  if (myLbl) myLbl.textContent = 'Checking connection…';
  if (mySub) mySub.textContent = 'Testing Supabase…';
  if (pingEl) pingEl.textContent = '…';

  var t0 = Date.now();
  var ok = false;
  try {
    var res = await sbClient().from('jo_history').select('*', { count: 'exact', head: true });
    var ping = Date.now() - t0;
    ok = !res.error;

    if (pingEl) {
      pingEl.textContent = ping + 'ms';
      pingEl.style.color = ping < 400 ? '#22c55e' : ping < 1200 ? '#f59e0b' : '#ef4444';
    }

    /* Schedule row count */
    var schedRes = await sbClient().from('jos_data').select('*', { count: 'exact', head: true });
    var histRes  = await sbClient().from('jo_history').select('*', { count: 'exact', head: true });
    if (schedEl) { schedEl.textContent = (!schedRes.error ? schedRes.count : 0) + ' dates'; schedEl.style.color='#22c55e'; }
    if (histEl)  { histEl.textContent  = (!histRes.error  ? histRes.count  : 0) + ' JOs';   histEl.style.color='#22c55e'; }

  } catch(e) { ok = false; }

  if (ok) {
    if (myLbl) myLbl.textContent = '✅ ' + _myPcName + ' — Connected to Supabase';
    if (mySub) mySub.textContent = 'This PC is live. Changes sync instantly to all other PCs.';
    if (myDot) { myDot.style.background='#22c55e'; myDot.style.boxShadow='0 0 12px #22c55e'; }
    if (myBar) { myBar.style.borderColor='rgba(34,197,94,0.3)'; myBar.style.background='rgba(34,197,94,0.08)'; }
    if (rtEl)  { rtEl.textContent = _presenceCh ? '🟢 ON' : '🟡 Starting'; rtEl.style.color = _presenceCh ? '#22c55e' : '#f59e0b'; }
    setSbLed('green');
  } else {
    if (myLbl) myLbl.textContent = '❌ ' + _myPcName + ' — Cannot reach Supabase';
    if (mySub) mySub.textContent = 'Check your internet connection.';
    if (myDot) { myDot.style.background='#ef4444'; myDot.style.boxShadow='0 0 12px #ef4444'; }
    if (myBar) { myBar.style.borderColor='rgba(239,68,68,0.3)'; myBar.style.background='rgba(239,68,68,0.08)'; }
    if (rtEl)  { rtEl.textContent = '🔴 OFF'; rtEl.style.color='#ef4444'; }
    setSbLed('red');
  }
  ...
  /* Prompt to name this PC if still default */
  var namePromptEl = document.getElementById('sb-name-prompt');
  if (namePromptEl) {
    namePromptEl.style.display = _myPcName === _myPcId ? 'block' : 'none';
  }
}

/* ── Hook into saves so other PCs get notified ── */
(function() {
  var _origSaveHist = window.saveHistory;
  if (typeof _origSaveHist === 'function') {
    window.saveHistory = function() { _origSaveHist.apply(this, arguments); sbBroadcastUpdate('J.O. History'); };
  }
  var _origJosSave = window.josSave;
  if (typeof _origJosSave === 'function') {
    window.josSave = function() { _origJosSave.apply(this, arguments); sbBroadcastUpdate('Schedule'); };
  }
  var _origSaveLoc = window.saveLocally;
  if (typeof _origSaveLoc === 'function') {
    window.saveLocally = function() { _origSaveLoc.apply(this, arguments); sbBroadcastUpdate('Worker Logs'); };
  }
})();

/* ── Start presence when page loads ── */
window.addEventListener('load', function() {
  setTimeout(function() {
    sbStartPresence();
    /* Background LED check */
    setTimeout(async function() {
      try {
        var res = await sbClient().from('jo_history').select('*', { count: 'exact', head: true });
        setSbLed(res.error ? 'red' : 'green');
        var myLbl = document.getElementById('sb-mypc-label');
        if (myLbl && !res.error) myLbl.textContent = '✅ ' + _myPcName + ' — Connected';
      } catch(e) { setSbLed('red'); }
    }, 1500);
  }, 1000);
});

/* ── Also insert the PC Name prompt inside the modal after DOM loads ── */
window.addEventListener('DOMContentLoaded', function() {
  var overlay = document.getElementById('sb-status-overlay');
  if (!overlay) return;
  var nameDiv = document.createElement('div');
  nameDiv.id = 'sb-name-prompt';
  nameDiv.style.cssText = 'display:none;margin:0 20px 16px;padding:14px 18px;background:rgba(251,191,36,0.1);border:1px solid rgba(251,191,36,0.25);border-radius:14px;';
  nameDiv.innerHTML = '<div style="font-size:12px;font-weight:800;color:#fbbf24;margin-bottom:8px;">💡 NAME THIS PC so you can tell it apart from others:</div>'
    + '<div style="display:flex;gap:8px;">'
    + '<input id="sb-pc-name-inp" placeholder="e.g. SCANNER PC, OFFICE PC, SUPERVISOR…" style="flex:1;padding:9px 13px;border-radius:10px;border:1px solid rgba(255,255,255,0.12);background:rgba(255,255,255,0.06);color:#f1f5f9;font-size:13px;font-weight:600;" />'
    + '<button onclick="sbSetMyName(document.getElementById(\'sb-pc-name-inp\').value.trim());document.getElementById(\'sb-name-prompt\').style.display=\'none\';runSbDiagnostic();" style="background:#0ea5e9;color:#fff;border:none;border-radius:10px;padding:9px 16px;font-weight:900;cursor:pointer;font-size:12px;">SAVE</button>'
    + '</div>';
  /* Insert before the bottom buttons */
  var closeBtn = overlay.querySelector('button[onclick="closeSbStatusModal()"]');
  if (closeBtn) {
    var btnRow = closeBtn.parentElement;
    overlay.querySelector('.\\0031 ').insertBefore(nameDiv, btnRow);
  } else {
    overlay.firstChild && overlay.firstChild.appendChild(nameDiv);
  }
});